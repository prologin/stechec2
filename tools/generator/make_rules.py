...
def make_rules(yaml_path: str, install_path: str) -> None:
    ...
    for gen in ["actions.hh", "api.cc", "api.hh", "constant.hh", "interface.cc", "rules.hh", "rules.cc"]:
        env.get_template(gen + ".jinja2").stream(yaml=conf).dump(os.path.join(install_path, gen))

    for action in conf["function"]:
        if "fct_action" in action and action["fct_action"]:
            env.get_template("action_template.hh.jinja2").stream(func=action).dump(os.path.join(install_path, "action_") + action["fct_name"] + ".hh")
            env.get_template("action_template.cc.jinja2").stream(func=action).dump(os.path.join(install_path, "action_") + action["fct_name"] + ".cc")
